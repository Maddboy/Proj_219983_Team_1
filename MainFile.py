...
def check_overlap(symm, vehicle_i_position_x, vehicle_i_position_y, vehicle_i_length, vehicle_i_width, try_position_x,
                  try_position_y, length, width):
    ok = False
    ...
    if symm == horr:
        ...
        if (
                0 <= vehicle_i_position_x - try_position_x <= length or 0 <= try_position_x - vehicle_i_position_x <= vehicle_i_length):

            # If the X coordinates as well as the Y coodinates overlap, then the vehicles overlap
            if (
                    0 <= vehicle_i_position_y - try_position_y <= width or 0 <= try_position_y - vehicle_i_position_y <= vehicle_i_width):
                ok = False
    ...
    # If the road is along the north-south direction
        ...
        # Check if the X coordinates overlap
        if (
                0 <= vehicle_i_position_x - try_position_x <= width or 0 <= try_position_x - vehicle_i_position_x <= vehicle_i_width):

            # If the X coordinates as well as the Y coodinates overlap, then the vehicles overlap
            if (
                    0 <= vehicle_i_position_y - try_position_y <= length or 0 <= try_position_y - vehicle_i_position_y <= vehicle_i_length):
                ok = False
...
def choose_position(road_no, lane_no, lane_specific_dimensions, lane_common_dimensions, length, width):
    # Try random values of coordinates in the lane
    try_position_x, try_position_y = generate_random_coordinates(roots[road_no].symm, lane_specific_dimensions,
                                                                 lane_common_dimensions, length, width)
    ...
    while (i < len(vehicles) and change_count <= 100):
        ...
        if vehicles[i].road_no == road_no and vehicles[i].lane_no == lane_no:
            ok = check_overlap(roots[road_no].symm, vehicles[i].position_x, vehicles[i].position_y, vehicles[i].length,
                               vehicles[i].width, try_position_x, try_position_y, length, width)
            if (not ok):
                try_position_x, try_position_y = generate_random_coordinates(roots[road_no].symm,
                                                                             lane_specific_dimensions,
                                                                             lane_common_dimensions, length, width)
                change_count += 1
                i = 0
                continue
...
def initialise_vehicles():
    ...
    tuple_of_colours = (
    'forestgreen', 'blue', 'red', 'black', 'brown', 'purple', 'navyblue', 'magenta', 'darkgreen', 'tomato2')
    for i in range(NO_OF_VEH):
        ...
        while (position_x, position_y) == (CHOOSE_POSITION_FAILED) and attempt_count < 100:
            ...
            position_x, position_y = choose_position(road_no, lane_no, lane_specific_dimensions, lane_common_dimensions,
                                                     length, width)
            attempt_count += 1
    ...
    if (node_id == -1):
        ...
        if veh_dir == "East" or veh_dir == "West":
            ...
            if abs(sig_pos - pos_y) < 30:
                ...
                return 0, sig_state, node_id  # 1 for in signal range and 0 for not in signal range


ref = 10
den_sig = [[set([]), set([]), set([]), set([])], [set([]), set([]), set([])], [set([]), set([]), set([])],
...
def motion(c):
    for i in range(NO_OF_VEH):
        k, signal_status, node_id = get_signal_info(vehicles[i].position_x, vehicles[i].position_y,
                                                    vehicles[i].direction, vehicles[i].road_no)

        # density precondition checking
        a, b, c = get_info_from_veh(vehicles[i].position_x, vehicles[i].position_y, vehicles[i].direction,
                                    vehicles[i].road_no)
        #  print(a," ",b)
        l1 = signals[a][b][2][0]
        l2 = signals[a][b][2][1]
        ...
        if (signal_status == 1 and k != 0):
            direction1 = get_random_direction(vehicles[i].position_x, vehicles[i].position_y, vehicles[i].direction,
                                              node_id)
...
while (not quit):
    ...
    for i in range(NO_OF_VEH):
        ...
        if roots[vehicles[i].road_no].symm == horr:
            vehicle_diagram = pg.Rect(vehicles[i].position_x, vehicles[i].position_y, vehicles[i].length,
                                      vehicles[i].width)
        else:
            vehicle_diagram = pg.Rect(vehicles[i].position_x, vehicles[i].position_y, vehicles[i].width,
                                      vehicles[i].length)
